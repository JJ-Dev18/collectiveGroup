...
const DarkModeToggle = () => {
  const { state, setTheme } = useContext(UiContext)
  const [checked, setChecked] = React.useState(state.theme.palette.mode === 'light' ? true : false);
  
  const handleTheme = (event: React.ChangeEvent<HTMLInputElement>)=> {
    // let theme = event.target.checked ? "dark" : 'light'
    setChecked(event.target.checked);
    setTheme(event.target.checked ? "light" : 'dark')
    Cookies.set("theme", event.target.checked ? "light" : 'dark')
  }
