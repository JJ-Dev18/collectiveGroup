...
export default function handler(
  ...
  switch (req.method) {
    ...
    case 'DELETE':
        return deleteSubscriptions(req,res)    
    default:
      res.status(400).json({
        message: "Bad request",
      });
...
const deleteSubscriptions = async ( req: NextApiRequest, res: NextApiResponse) => {
    ...
    const {
      ...
      try {
          await prisma.subscriptionDetailPackage.deleteMany({
              where:{
                  subscriptionId: id
              }
          })  
...
const updatedUser = async ( req: NextApiRequest, res: NextApiResponse) => {
    ...
    const {
      ...
      } = req.body as { id: number; role: "USER" | "ADMIN" | undefined };
